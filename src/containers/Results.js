...
function Results() {
  ...
  const possibleResponses = useMemo(
    () => ({
      rsp34459: (
  ...
  const returnAnswer = (answer) => {
    if (!results[answer.answerId]) return false;
    ...
    return (
      <div
        key={answer.answerId}
        className={`answer ${isValid ? "valid" : "invalid"}`}
      >
        <p className="answer-title">{answer.answerTitle}</p>
        <div className="answer-placeholder">{possibleResponses[response]}</div>
      </div>
    );
  };
